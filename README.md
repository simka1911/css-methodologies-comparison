...
## Instructions
Clone or download this project on your computer, checkout **develop** branch, open up the terminal, navigate to the root folder of this project (location of package.json file) and run ```npm install``` command. To start the webpack dev server, just run the script ```npm run start```, and index page will be immediately opened in the browser. From there you can navigate to three different html pages, one for each methodology. All markup, styles and javascript are found in the **src** folder and they are the ones which should be edited. The browser should refresh whenever you save changes in your code. If you make some changes inside the webpack.config.js file, you will need to start the script again.
#### Additional info:
